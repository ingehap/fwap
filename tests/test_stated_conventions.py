...
An ordering or a direction is the class of claim this solver gets
wrong most quietly. "``alpha_qP`` is the smaller root", "row 3 is
``sigma_rz``", "index 0 is the fundamental", "the march ascends
frequency" -- each of these can be false while every numerical test
still passes, because the code is right and only the *label* is wrong.
Nothing in a determinant, a residual, or a dispersion comparison
touches a label.
