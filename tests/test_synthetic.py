...
def test_array_geometry_offsets_and_time():
    ...
    g = ArrayGeometry(n_rec=4, tr_offset=3.0, dr=0.1, dt=1.0e-5, n_samples=100)
...
def test_array_geometry_from_imperial_matches_default():
    ...
    g_imp = ArrayGeometry.from_imperial(
        n_rec=g_metric.n_rec,
...
def test_mode_with_dispersion_receives_array_input():
    ...
    geom = ArrayGeometry(n_rec=4, tr_offset=3.0, dr=0.1, dt=1.0e-5, n_samples=256)
    mode = Mode(
        name="D", slowness=1.0 / 2500.0, f0=3000.0, amplitude=1.0, dispersion=disp
    )
