...
def test_eaton_exponent_amplifies_departure_from_normal():
    ...
    P_n3 = pore_pressure_eaton(
        ...
        s_obs,
        s_normal,
        hydrostatic_pressure_pa=P_hydro,
        eaton_exponent=3.0,
    )
...
def test_eaton_round_trip_with_overburden_log():
    ...
    np.testing.assert_allclose(P_pore[outside], P_hydro[outside], rtol=1.0e-12)
...
def test_window_width_and_drillability_properties():
    ...
    w_drillable = safe_mud_weight_window(
        sigma_H=50.0e6,
        sigma_h=45.0e6,  # low anisotropy
        pore_pressure=20.0e6,
        ucs=10.0e6,  # very weak rock
    )
    ...
    w_impossible = safe_mud_weight_window(
        sigma_H=80.0e6,
        sigma_h=30.0e6,  # high anisotropy
        pore_pressure=40.0e6,
        ucs=80.0e6,  # strong rock
    )
...
def _bowers_unloading_forward(
    sigma_eff_pa, sigma_max_pa, V_ml=BOWERS_V_ML, A=BOWERS_A, B=BOWERS_B, U=BOWERS_U
):
...
def test_bowers_loading_higher_velocity_gives_lower_pore_pressure():
    ...
    V_low = 1700.0
    V_high = 2000.0
    P_p_low_V = float(pore_pressure_bowers(np.array([sigma_v]), np.array([V_low]))[0])
    P_p_high_V = float(pore_pressure_bowers(np.array([sigma_v]), np.array([V_high]))[0])
...
def test_inclined_breakout_increases_with_inclination():
    ...
    assert all(P_iota[i] <= P_iota[i + 1] + 1.0e-3 for i in range(len(P_iota) - 1))
...
def test_inclined_breakout_horizontal_well_azimuth_dependence():
    ...
    base = dict(
        sigma_v=INCL_SIGMA_V,
        sigma_H=INCL_SIGMA_H,
        sigma_h=INCL_SIGMA_H_MIN,
        pore_pressure=INCL_PORE_PRESSURE,
        ucs=INCL_UCS,
        friction_angle_deg=INCL_FRICTION,
        well_inclination_deg=90.0,
        n_azimuth=360,
    )
...
def test_inclined_wall_stresses_periodic_over_360_deg():
    ...
    s_t, s_z, s_tz, _ = inclined_wellbore_wall_stresses(
        ...
        INCL_SIGMA_H,
        INCL_SIGMA_H_MIN,
        well_inclination_deg=45.0,
        well_azimuth_deg=30.0,
        azimuth_around_wall_deg=azimuths,
        mud_pressure=30.0e6,
        poisson=INCL_NU,
    )
...
def test_inclined_breakdown_decreases_with_inclination():
    ...
    assert all(P_break[i] >= P_break[i + 1] - 1.0e3 for i in range(len(P_break) - 1))
...
def test_inclined_window_narrows_with_inclination():
    ...
    assert all(widths[i] >= widths[i + 1] - 1.0e3 for i in range(len(widths) - 1))
