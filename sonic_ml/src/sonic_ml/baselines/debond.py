...
def gap_thickness(
    ...
    The single reader for this quantity: the baseline scores against it, the
    learned inverse takes its ``log10`` as a target, and the benchmark harness
    scores both against it. Three copies of a two-line lookup is three chances
    for one of them to drift onto the wrong column.
    ...
    ------
    ValueError
        If the bundle carries no layer stack, or no layer is ``gap_name``.
    """
    if bundle.layer_params is None:
        raise ValueError("bundle carries no layer stack")
    if gap_name not in bundle.layer_names:
        raise ValueError(
            f"no {gap_name!r} layer in {bundle.layer_names!r}; this is not a "
            "debonded dataset. One is generated by "
            "gen_surrogate_dataset.py --debonded"
        )
    index = bundle.layer_names.index(gap_name)
    thickness = LAYER_PARAM_NAMES.index("thickness")
    return np.asarray(bundle.layer_params[:, index, thickness], dtype=float)
...
class CrackWaveThicknessBaseline:
    ...
    def truth(self, bundle: DatasetBundle) -> np.ndarray:
        ...
        ndarray, shape (n_samples,)
            Gap width (m).
        """
        return gap_thickness(bundle, gap_name=self.gap_name)
