...
def demo_stc_picker(figdir: str = "figures", show: bool = False) -> None:
    ...
    for name, p in picks.items():
        ...
        logger.info(
            ...
            name, p.slowness / US_PER_FT, 1.0 / p.slowness,
            p.coherence, amp_str,
        )
    logger.info("  Truth: Vp=%.0f  Vs=%.0f  Vst=%.0f", Vp, Vs, Vst)

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    _wiggle(axes[0], data, geom.t, xmax=3.5e-3,
            title=(f"Synthetic monopole gather\n"
                   f"Vp={Vp:.0f}  Vs={Vs:.0f}  Vst={Vst:.0f} m/s"))
    ax = axes[1]
    pcm = ax.pcolormesh(res.time * 1e3, res.slowness / US_PER_FT,
                        np.nan_to_num(res.coherence),
                        shading="auto", cmap="viridis", vmin=0, vmax=1)
    plt.colorbar(pcm, ax=ax, label="Coherence")
    for p in picks.values():
        ax.plot(p.time * 1e3, p.slowness / US_PER_FT,
                "o", mfc="none", mec="red", mew=2, ms=10, label=p.name)
    ax.legend()
    ax.set_xlim(0, 3.5)
    ax.set_xlabel("Time (ms)"); ax.set_ylabel("Slowness (us/ft)")
...
def demo_pseudo_rayleigh(figdir: str = "figures", show: bool = False) -> None:
    ...
    """
    import matplotlib.pyplot as plt
    logger.info("=== Demo: STC + 4-mode picker (P / S / pseudo-Rayleigh / Stoneley) ===")
    ...
    geom = ArrayGeometry(n_rec=8, tr_offset=3.0, dr=0.1524,
                         dt=1.0e-5, n_samples=2048)
    modes = monopole_formation_modes(
        vp=Vp, vs=Vs, v_stoneley=Vst,
        v_fluid=v_fluid, f_pr=f_pr, pr_amp=2.0)
    ...
    logger.info("  Truth: Vp=%.0f  Vs=%.0f  v_fluid=%.0f  Vst=%.0f",
                Vp, Vs, v_fluid, Vst)

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    _wiggle(axes[0], data, geom.t, xmax=3.5e-3,
            title=(f"Synthetic 4-mode monopole gather\n"
...
def demo_wave_separation(figdir: str = "figures", show: bool = False) -> None:
    ...
    p_only  = fk_filter(data, geom.dt, geom.dr,
                        slow_min=1.0/5500, slow_max=1.0/3600, taper_width=0.3)
    s_only  = fk_filter(data, geom.dt, geom.dr,
                        slow_min=1.0/3000, slow_max=1.0/2100, taper_width=0.3)
    st_only = fk_filter(data, geom.dt, geom.dr,
                        slow_min=1.0/1700, slow_max=1.0/1100, taper_width=0.3)
    ...
    plt.suptitle("Wave separation -- f-k filter (top) vs SVD/K-L (bottom)",
                 fontsize=11)
    plt.tight_layout()
    _savefig(fig, figdir, "demo_wave_separation.png", show=show)


def demo_tau_p_separation(figdir: str = "figures",
    ...
    # comparison.
    p_band  = tau_p_filter(data, geom.dt, geom.offsets,
                           slow_min=1.0 / 5500, slow_max=1.0 / 3600,
                           n_slowness=181, taper_width=0.3)
    s_band  = tau_p_filter(data, geom.dt, geom.offsets,
                           slow_min=1.0 / 3000, slow_max=1.0 / 2100,
                           n_slowness=181, taper_width=0.3)
    st_band = tau_p_filter(data, geom.dt, geom.offsets,
                           slow_min=1.0 / 1700, slow_max=1.0 / 1100,
                           n_slowness=181, taper_width=0.3)
    ...
    plt.suptitle("Wave separation -- tau-p (slant-stack / linear Radon)",
                 fontsize=11)
    plt.tight_layout()
    _savefig(fig, figdir, "demo_tau_p_separation.png", show=show)


def demo_intercept_time(figdir: str = "figures",
                        show: bool = False) -> None:
    import matplotlib.pyplot as plt
    logger.info("=== Demo: Intercept-time inversion "
                "(Coppens & Mari, 1995) ===")
    ...
    r_seg = solve_intercept_time(
        travel_times, off_vec, src_depth, rec_depth, n_d,
        depth_axis=depth_axis, mean_delay_zero=True,
        smooth_s=5.0e3, smooth_src=1.0e3, smooth_rec=1.0e3,
        delay_l2=1.0e2, method="segmented")

    logger.info("  midpoint  RMS %.2f us   mean s %.2f us/ft (truth %.2f)",
                r_mp.rms_residual * 1e6,
                np.mean(r_mp.slowness) / US_PER_FT,
                s_bg / US_PER_FT)
    logger.info("  segmented RMS %.2f us   mean s %.2f us/ft",
    ...
    ax = axes[0]
    ax.plot(r_mp.slowness / US_PER_FT, depth_axis, "b-",
            alpha=0.5, label="Inverted (midpoint)")
    ax.fill_betweenx(depth_axis,
                     (r_mp.slowness - r_mp.sigma_slowness) / US_PER_FT,
                     (r_mp.slowness + r_mp.sigma_slowness) / US_PER_FT,
                     alpha=0.15, color="b")
    ax.plot(r_seg.slowness / US_PER_FT, depth_axis, "g-",
            label="Inverted (segmented)")
    ax.fill_betweenx(depth_axis,
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Slowness (us/ft)"); ax.set_ylabel("Depth (m)")
    ax.set_title("Virgin formation slowness\n(shaded = +/- sigma)")
    ax.grid(alpha=0.3); ax.legend()

    ax = axes[1]
    ax.plot(r_mp.delay_src * 1e6, depth_axis, "b-", alpha=0.5,
            label="Inverted (midpoint)")
    ax.plot(r_seg.delay_src * 1e6, depth_axis, "g-",
            label="Inverted (segmented)")
    ...
    ax = axes[2]
    ax.plot(r_mp.delay_rec * 1e6, depth_axis, "b-", alpha=0.5,
            label="Inverted (midpoint)")
    ax.plot(r_seg.delay_rec * 1e6, depth_axis, "g-",
            label="Inverted (segmented)")
    ax.plot(delay * 1e6, depth_axis, "k--", label="Truth")
    ax.invert_yaxis()
...
def demo_dipole(figdir: str = "figures", show: bool = False) -> None:
    ...
    res_lowf = narrow_band_stc(data, dt=geom.dt, offsets=geom.offsets,
                               f_lo=500.0, f_hi=1500.0,
                               slowness_range=(50e-6, 800e-6),
                               n_slowness=151, window_length=1.5e-3,
                               time_step=4)
    # Wide-band STC: high-f dispersion biases the estimate away from Vs.
    res_wide = narrow_band_stc(data, dt=geom.dt, offsets=geom.offsets,
                               f_lo=500.0, f_hi=10_000.0,
                               slowness_range=(50e-6, 800e-6),
                               n_slowness=151, window_length=1.5e-3,
                               time_step=4)
    ...
    curve_fu = phase_slowness_from_f_k(
        data, dt=geom.dt, offsets=geom.offsets,
        f_range=(500.0, 8000.0), method="frequency_unwrap")
    curve_mp = phase_slowness_matrix_pencil(
        data, dt=geom.dt, offsets=geom.offsets,
        f_range=(500.0, 8000.0))
    s_avg = shear_slowness_from_dispersion(curve_fu,
                                           f_lo=1500.0, f_hi=2500.0,
                                           quality_threshold=0.3)

    logger.info("  True Vs                = %.0f m/s (%.2f us/ft)",
                Vs, s_true / US_PER_FT)
    logger.info("  Narrow-band STC Vs     = %.0f (%.2f us/ft)",
                1.0 / s_lowf, s_lowf / US_PER_FT)
    logger.info("  Wide-band STC Vs       = %.0f (biased low by high-f "
                "dispersion)", 1.0 / s_wide)
    logger.info("  Dispersive STC Vs      = %.0f (Kimball 1998)",
                1.0 / s_disp)
    logger.info("  Low-f asymptote Vs     = %.0f", 1.0 / s_avg)

    fig, axes = plt.subplots(2, 2, figsize=(13, 9))
    _wiggle(axes[0, 0], data, geom.t, xmax=geom.t[-1] * 0.5,
            title="Dispersive flexural gather")

    ax = axes[0, 1]
    f_grid = np.linspace(50, 8000, 200)
    ax.plot(f_grid, disp(f_grid) / US_PER_FT,
            "k--", label="True s(f)")
    ax.plot(curve_fu.freq, curve_fu.slowness / US_PER_FT, "b.-",
            alpha=0.6, ms=3, label="freq_unwrap")
    ax.plot(curve_mp.freq, curve_mp.slowness / US_PER_FT, "r.",
            alpha=0.5, ms=3, label="matrix_pencil")
    ax.axhline(1.0 / Vs / US_PER_FT, color="g", ls=":",
    ...
    pcm = ax.pcolormesh(res_disp.time * 1e3,
                        res_disp.slowness / US_PER_FT,
                        rho_disp, shading="auto",
                        cmap="viridis", vmin=0, vmax=1)
    plt.colorbar(pcm, ax=ax, label="Coherence")
    ax.axhline(1.0 / Vs / US_PER_FT, color="r", ls="--",
               label="True shear")
    ax.set_xlabel("Time (ms)")
    ax.set_ylabel("SHEAR slowness (us/ft)")
    ax.set_title("Dispersive STC (Kimball, 1998)\n"
                 "-- shear slowness, not phase slowness")
    ax.legend()

    ax = axes[1, 1]
    ax.plot(curve_fu.freq, curve_fu.quality, "b.-",
            label="freq_unwrap")
    ax.plot(curve_mp.freq, curve_mp.quality, "r.-",
            label="matrix_pencil")
    ax.set_xlabel("Frequency (Hz)"); ax.set_ylabel("Fit quality")
    ax.set_ylim(0, 1.05)
...
def demo_dip(figdir: str = "figures", show: bool = False) -> None:
    ...
    data, dt, ax_off, az, a, slow = synthesize_azimuthal_arrival(
        ...
        dip=true_dip, azimuth=true_az, f0=8000.0, noise=0.02, seed=3)
    ...
    logger.info("  True   dip=%6.2f  az=%6.2f",
                np.rad2deg(true_dip), np.rad2deg(true_az))
    logger.info("  Recov. dip=%6.2f  az=%6.2f  coh=%.3f  refined=%s",
                np.rad2deg(result.dip), np.rad2deg(result.azimuth),
                result.coherence, result.refined)
    ...
    ax.set_xlim(0.2, 0.8)
    ax.set_ylim(-0.7, data.shape[0] - 0.3); ax.invert_yaxis()
    ax.set_xlabel("Time (ms)"); ax.set_ylabel("Receiver (by azimuth)")
    ax.set_title("Azimuthal array -- arrival time is a cosine\n"
                 "of receiver azimuth for a dipping bed")

    ax = axes[1]
    pcm = ax.pcolormesh(np.rad2deg(result.azimuth_axis),
                        np.rad2deg(result.dip_axis), result.surface,
                        shading="auto", cmap="viridis")
    plt.colorbar(pcm, ax=ax, label="Coherence")
    ax.plot(np.rad2deg(result.azimuth), np.rad2deg(result.dip),
            "ro", ms=12, mfc="none", mew=2,
            label=f"Recovered ({'refined' if result.refined else 'grid'})")
    ax.plot(np.rad2deg(true_az), np.rad2deg(true_dip),
            "w+", ms=14, mew=2, label="Truth")
    ax.set_xlabel("Azimuth (deg)"); ax.set_ylabel("Dip (deg)")
    ax.set_title("Coherence over (dip, azimuth)"); ax.legend()
    plt.suptitle("Dip / azimuth from azimuthal acoustic array\n"
...
def demo_attenuation(figdir: str = "figures",
                     show: bool = False) -> None:
    import matplotlib.pyplot as plt
    logger.info("=== Demo: Attenuation (Q) from array sonic ===")
    geom = ArrayGeometry(n_rec=12, tr_offset=3.0, dr=0.1524,
                         dt=1.0e-5, n_samples=2048)
    Vp = 4000.0
    Q_true = 50.0
    f0 = 15_000.0
    ...
    res_c = centroid_frequency_shift_Q(
        ...
        window_length=4.0e-4, f_range=(5_000.0, 30_000.0),
        pick_intercept=t0)
    res_r = spectral_ratio_Q(
        data, dt=geom.dt, offsets=geom.offsets, slowness=1.0 / Vp,
        window_length=4.0e-4, f_range=(5_000.0, 25_000.0),
        pick_intercept=t0)
    logger.info("  True Q             = %.1f", Q_true)
    logger.info("  Centroid-shift  Q  = %.1f +/- %.1f",
                res_c.q, res_c.q_sigma)
    logger.info("  Spectral-ratio  Q  = %.1f +/- %.1f",
                res_r.q, res_r.q_sigma)

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    _wiggle(axes[0], data, geom.t, xmax=3.0e-3,
            title=f"Attenuated gather (true Q={Q_true:.0f})")
    ax = axes[1]
    tt_arr = t0 + geom.offsets / Vp
    ax.plot(tt_arr * 1e6, res_c.diagnostic["fc"] / 1e3,
            "bo-", label="centroid fc(t)")
    ...
    ax.set_xlabel("Travel time (us)"); ax.set_ylabel("Centroid freq (kHz)")
    ...
    for i in range(1, n_rec, 2):
        ...
        w_i = np.hanning(L) * data[i,
                                    int(round(ti / geom.dt)):
                                    int(round(ti / geom.dt)) + L]
        w_r = np.hanning(L) * data[i_ref,
                                    int(round(t_ref / geom.dt)):
                                    int(round(t_ref / geom.dt)) + L]
        ...
        ax.plot(fi[mask] / 1e3, np.log(ampi[mask] / ampr[mask]),
                label=f"rec {i}/{i_ref}", alpha=0.6)
    ax.set_xlabel("Frequency (kHz)")
    ax.set_ylabel("log |A_i / A_0|")
    ax.grid(alpha=0.3); ax.legend(fontsize=8)

    plt.suptitle("Attenuation (Q) estimation\n"
...
def demo_alford(figdir: str = "figures", show: bool = False) -> None:
    ...
    logger.info("  Recovered axis:  %.2f deg  (cross_en_ratio=%.3e)",
                np.rad2deg(res.angle), res.cross_energy_ratio)

    fig, axes = plt.subplots(2, 2, figsize=(13, 8))
    axes[0, 0].plot(t * 1e3, xx, "k"); axes[0, 0].set_title("xx")
    axes[0, 1].plot(t * 1e3, xy, "k"); axes[0, 1].set_title("xy")
    axes[1, 0].plot(t * 1e3, yx, "k"); axes[1, 0].set_title("yx")
    axes[1, 1].plot(t * 1e3, yy, "k"); axes[1, 1].set_title("yy")
    for ax in axes.ravel():
        ax.set_xlim(0.5, 2.0)
        ax.set_xlabel("Time (ms)"); ax.grid(alpha=0.3)
    plt.suptitle("Input cross-dipole tensor (in tool x,y frame)",
                 fontsize=11)
    plt.tight_layout()
    ...
    axes[0].set_title(f"After Alford rotation\n"
    ...
    axes[1].axvline(np.rad2deg(res.angle), color="r", ls="--",
                    label=f"recovered {np.rad2deg(res.angle):.1f} deg")
    axes[1].axvline(np.rad2deg(true_angle), color="g", ls=":",
    ...
    plt.suptitle("Cross-dipole Alford rotation -- Alford (1986)",
                 fontsize=11)
    plt.tight_layout()
...
def demo_lwd(figdir: str = "figures", show: bool = False) -> None:
    ...
    * ``demo_lwd_quadrupole.png`` -- the quadrupole-tool workflow.
    ...
    cleaned = notch_slowness_band(
        data, dt=geom.dt, offsets=geom.offsets,
        slow_min=collar_slow * 0.85, slow_max=collar_slow * 1.15,
        n_slowness=181, taper_width=0.15,
    )
    surf_dirty = stc(data, dt=geom.dt, offsets=geom.offsets,
                     slowness_range=(30 * US_PER_FT, 360 * US_PER_FT),
                     n_slowness=181, window_length=4.0e-4, time_step=2)
    surf_clean = stc(cleaned, dt=geom.dt, offsets=geom.offsets,
                     slowness_range=(30 * US_PER_FT, 360 * US_PER_FT),
                     n_slowness=181, window_length=4.0e-4, time_step=2)
    from fwap.picker import DEFAULT_PRIORS
    three_mode_priors = {m: DEFAULT_PRIORS[m] for m in ("P", "S", "Stoneley")}
    picks_clean = pick_modes(surf_clean, priors=three_mode_priors,
                             threshold=0.4)

    logger.info("  Monopole + collar contamination at %.0f us/ft, "
                "Vp=%.0f Vs=%.0f Vst=%.0f m/s",
                collar_slow / US_PER_FT, Vp, Vs, Vst)
    logger.info("  After collar-band notch:")
    for name in ("P", "S", "Stoneley"):
        if name in picks_clean:
            p = picks_clean[name]
            logger.info("    %-9s slowness=%6.2f us/ft  V=%6.0f m/s  coh=%.3f",
                        name, p.slowness / US_PER_FT, 1.0 / p.slowness,
                        p.coherence)
    ...
    for ax, surf, title in (
        (axes[0], surf_dirty,
         "Pre-rejection: collar peak dominates"),
        (axes[1], surf_clean,
         "Post-rejection: formation P/S/Stoneley recovered"),
    ):
        pcm = ax.pcolormesh(surf.time * 1e3, surf.slowness / US_PER_FT,
                            np.nan_to_num(surf.coherence),
                            shading="auto", cmap="viridis", vmin=0, vmax=1)
        plt.colorbar(pcm, ax=ax, label="Coherence")
        ax.axhline(collar_slow / US_PER_FT, color="orange", ls=":",
                   alpha=0.7, label="Collar slowness")
        for V, lbl in ((Vp, "P"), (Vs, "S"), (Vst, "Stoneley")):
            ax.axhline((1.0 / V) / US_PER_FT, color="white", ls="--",
                       alpha=0.4)
        ax.set_xlim(0, 5.0)
        ax.set_ylim(30, 320)
        ax.set_xlabel("Time (ms)")
    ...
    if picks_clean:
        for p in picks_clean.values():
            axes[1].plot(p.time * 1e3, p.slowness / US_PER_FT,
                         "o", mfc="none", mec="red", mew=2, ms=10)
    plt.suptitle("LWD collar-mode rejection -- monopole, "
                 "Tang & Cheng (2004) sect. 2.4",
                 fontsize=11)
    plt.tight_layout()
    ...
    ax.plot(theta_grid, np.cos(2.0 * np.deg2rad(theta_grid)),
            "k--", alpha=0.5, label=r"$\cos(2\theta)$  (m=2 source pattern)")
    ax.set_xlabel("Receiver azimuth (deg)")
    ax.set_ylabel("Amplitude at formation-arrival sample")
    ax.set_title("Quadrupole ring response\n"
                 f"Vs = {Vs_q:.0f} m/s,  8 azimuthal receivers")
    ax.legend()
    ax.grid(alpha=0.3)

    ax = axes[1]
    pcm = ax.pcolormesh(surf_q.time * 1e3, surf_q.slowness / US_PER_FT,
                        np.nan_to_num(surf_q.coherence),
                        shading="auto", cmap="viridis", vmin=0, vmax=1)
    plt.colorbar(pcm, ax=ax, label="Coherence")
    ax.axhline((1.0 / Vs_q) / US_PER_FT, color="white", ls="--",
               alpha=0.6, label="True Vs")
    ...
    plt.suptitle("LWD quadrupole stack -- m=2 source / receiver geometry, "
...
def demo_las_roundtrip(figdir: str = "figures", show: bool = False) -> None:
    ...
    write_las(path, depth, curves,
              depth_unit="M",
              well_name="FWAP_DEMO",
              well={"COMP": "fwap", "SRVC": "fwap.demos"})
    logger.info("  wrote %s (%d curves, %d depths)",
                path, len(curves), depth.size)

    loaded = read_las(path)
    logger.info("  read  %s (%d curves, step=%.4f m)",
                path, len(loaded.curves), loaded.step)
    ...
    for name, orig in curves.items():
        ...
        drift = float(np.sqrt(np.mean(
            (loaded.curves[name][mask] - orig[mask]) ** 2)))
...
def demo_dlis_roundtrip(figdir: str = "figures", show: bool = False) -> None:
    ...
    logger.info("  read  %s (frame=%s, index_type=%s, step=%.4f m)",
                path, loaded.frame_name, loaded.index_type, loaded.step)
    ...
    plt.suptitle(
        "DLIS I/O round-trip -- binary RP66 v1, bit-exact float64 storage",
        fontsize=11)
    plt.tight_layout()
...
def demo_segy_roundtrip(figdir: str = "figures", show: bool = False) -> None:
    ...
    write_segy(path, data.astype(np.float32), dt=geom.dt,
               offsets=offset_mm,
               textual_header="fwap demo_segy_roundtrip synthesized gather")
    logger.info("  wrote %s (%d traces, %d samples, dt=%.1f us)",
                path, data.shape[0], data.shape[1], geom.dt * 1e6)

    loaded = read_segy(path)
    logger.info("  read  %s (%d traces, %d samples, dt=%.1f us)",
                path, loaded.n_traces, loaded.n_samples, loaded.dt * 1e6)
    ...
    logger.info("  data round-trip max|err|=%.3e (rel %.3e)",
                max_abs_err, rel_err)
    ...
    res = stc(loaded.data.astype(float),
              dt=loaded.dt, offsets=geom.offsets,
              slowness_range=(30 * US_PER_FT, 360 * US_PER_FT),
              n_slowness=121, window_length=4.0e-4, time_step=2)
    picks = pick_modes(res, threshold=0.4)
    for name, p in picks.items():
        logger.info("    %-9s V=%6.0f m/s  coh=%.3f",
                    name, 1.0 / p.slowness, p.coherence)
    logger.info("  Truth: Vp=%.0f  Vs=%.0f  Vst=%.0f", Vp, Vs, Vst)

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    _wiggle(axes[0], data, geom.t, xmax=3.5e-3,
            title="Original synthetic gather")
    _wiggle(axes[1], loaded.data, geom.t, xmax=3.5e-3,
            title=(f"After SEG-Y round-trip\n"
                   f"max |diff|={max_abs_err:.2e}"))
    plt.suptitle(
        "SEG-Y I/O round-trip -- write, read, re-process, compare",
        fontsize=11)
    plt.tight_layout()
