...
`tests/test_solver_*.py` so a fix shows up as a failure.
...
tests in `tests/test_solver_*.py` pin the disagreement against the
...
`tests/test_solver_*.py` pins this comparison so the attribution
