...
| **A.8 The SV column of the layer matrices is not a solution** | *New, and the first defect found by checking fwap's **formulation** rather than its output.* Schmitt & Cheng print all 36 elements of the layer matrix `T(n, j, r)` in their appendix (pp. 235-236, "the ones numerically used"). Transcribed and checked against Hooke's law it is self-consistent to 1e-11. Written in that basis, the P and SH columns of `_layer_e_matrix_n1` / `_n2` (and their complex twins) are **exact** — coefficients constant to ~1e-13 across a 4 cm change in radius. The two **SV columns drift by 24-86 %**, so they are not solutions of the elastodynamic equations at all: a solution is a *fixed* combination of any basis for the solution space. The cause is the ansatz, not the algebra — fwap builds SV from a vector potential with only an azimuthal component, and the cylindrical vector Laplacian couples the radial and azimuthal components through a term ∝ `n` that such an ansatz cannot cancel (the radial equation becomes Bessel of order `sqrt(n²+1)`). At `n=0` that term vanishes, which is why `_layer_e_matrix_n0` is clean and why the Stoneley path ties figure 8a at 0.04 %. **Cost, measured end-to-end**: a determinant assembled only from the appendix tracks figure 15's invaded-zone curve at **0.24 % rms, median −0.22 %, errors of both signs**, where `flexural_dispersion_layered` reads **1.49 % rms, median −1.49 %, low at all eight frequencies**. So figure 15's "1.47 % rms", read when measured as *the layered propagator is sound*, is nearly sound — and this is what the remaining 1.5 % is. Almost certainly also the root of A.7: a propagator built from a non-solution has no reason to produce a clean determinant. Fix is mechanical (the appendix gives the correct column) but wide: four layer matrices, both propagators, the cased determinants at n=1 and n=2, and the single-layer path, which carries the same ansatz in separate code. Wants figures 20-21 digitised to validate. Pinned by twelve tests in `tests/test_schmitt_appendix.py`, not fixed. |
| **A.7 The cased `n=2` determinant is noise-dominated** | *New, exposed by fixing A.2 — the narrow bracket had been hiding it.* Scanned across the corrected `(V_f, V_S)` window, `_modal_determinant_n2_cased_complex` produces **~90 sign changes at 12 kHz** on figure 14's model where the physics supports a handful, and **10-33** even when the single layer is set identical to the formation — a configuration that is physically the open hole, where the un-cased 4×4 gives one clean root. They come as near-duplicate pairs straddling the true value (2084.0 and 2085.0 against the open hole's 2084.9): catastrophic cancellation in the propagator chain, not a mode spectrum. The marcher now declines to choose past `_FAST_FLEXURAL_MAX_CASED_ROOTS` crossings, so the layered `n=2` fast path returns **NaN rather than a root drawn from noise** — quiet, not fixed. `n=1` cased is unaffected (figure 12a tracks to 3.8 %), so this is specific to the `n=2` chain. The intended route is the delta-matrix / Abo-Zena reformulation already on the list as the A.5 residue, which exists precisely to remove this cancellation; it moves from *optional* to *blocking this path*. Pinned by two tests. |
