...
| **A.9 The slow-formation cased `n >= 1` dispatch has no leaky search** | *New, exposed by fixing A.8.* Behind a steel casing the cased dipole and screw modes are faster than a slow formation's shear speed, so they radiate into it and the real-valued bound-regime determinant has no root: `flexural_dispersion_layered` and `quadrupole_dispersion_layered` return all-NaN for slow-formation cased stacks. The modes are present and findable — `Im(det)` of `_modal_determinant_n1_cased_complex` with `leaky_p`/`leaky_s` set crosses zero at **830 m/s at 3 kHz and 877 at 6 kHz** against `V_S = 800` for the standard steel + cement fixture — but the marcher that would find them is wired only for fast formations, where its window `(V_f, V_S)` is non-empty. A slow-formation cased search needs the window `(V_S_formation, min(layer V_S, V_f))` instead, plus the same fundamental-selection care A.2 needed: the scan shows four crossings at 9 kHz. There is no published cased-hole dispersion curve in Schmitt & Cheng to validate against, so this wants figures 20-21 digitised first, or another source. Fast-formation cased stacks are unaffected (2600 → 1506 m/s over 2-6.5 kHz), as are invaded-zone stacks at any layer count. Pinned by `test_cased_slow_formation_dipole_is_leaky_not_bound`. |
| **A.7 The cased `n=2` determinant is noise-dominated** | *New, exposed by fixing A.2 — the narrow bracket had been hiding it.* Scanned across the corrected `(V_f, V_S)` window, `_modal_determinant_n2_cased_complex` produces **~90 sign changes at 12 kHz** on figure 14's model where the physics supports a handful, and **10-33** even when the single layer is set identical to the formation — a configuration that is physically the open hole, where the un-cased 4×4 gives one clean root. They come as near-duplicate pairs straddling the true value (2084.0 and 2085.0 against the open hole's 2084.9): catastrophic cancellation in the propagator chain, not a mode spectrum. The marcher now declines to choose past `_FAST_FLEXURAL_MAX_CASED_ROOTS` crossings, so the layered `n=2` fast path returns **NaN rather than a root drawn from noise** — quiet, not fixed. `n=1` cased is unaffected (figure 12a tracks to 3.8 %), so this is specific to the `n=2` chain. The intended route is the delta-matrix / Abo-Zena reformulation already on the list as the A.5 residue, which exists precisely to remove this cancellation; it moves from *optional* to *blocking this path*. Pinned by two tests. **A.8 was not the cause.** The hypothesis recorded when A.8 was found was that a propagator built from a non-solution has no reason to yield a clean determinant. It was wrong: with the SV column corrected, the same layer-equals-formation scan gives an identical **430 sign changes on a 1200-point grid at 12 kHz**, before and after, to the sample. The cancellation is in the propagator chain, not in the column fed into it, so the delta-matrix reformulation remains the only route. |
