...
| **A.8 The SV column is not a solution — every `n >= 1` determinant** | *Scope enlarged and the fix demonstrated, then reverted as half a change.* Schmitt & Cheng's appendix (pp. 235-268, matrix on 235-236) prints all 36 elements of the layer matrix; transcribed and self-consistent under Hooke's law to 1e-11. Against it, fwap's P and SH columns are exact (r-independent to 1e-13) and the **SV columns drift 24-86 %**, so they are not solutions. Cause is the ansatz: an azimuthal-only vector potential, which the cylindrical vector Laplacian does not admit for `n >= 1` (the coupling term carries a factor `n`; at `n = 0` it vanishes, and `n = 0` is clean). **It is not confined to the layered paths.** Rebuilding the open-hole problem from the appendix and scoring both against figure 8a's three published curves for one rock: Stoneley **0.03 % vs fwap 0.03 %**, flexural **0.08 % vs 1.30 %**, screw **0.06 % vs 0.43 %**. So the published curves are reproducible to 0.06 %, and *the "±1 % digitisation floor" this project has quoted for every `n >= 1` tie was fwap's model error, not the scan*. **The correction is derived and measured**, not hypothesised: with `A = s K_{n-1} + n K_n / a`, `M13 = kz A`; `M23 = -2 kz mu (s^2 K_n + s K_{n-1}/a + n(n+1) K_n/a^2)`; `M33 = 2 kz mu (n/a)(s K_{n-1} + (n+1) K_n/a)`; `M43 = (2 kz^2 - kS^2) mu A` — verified against the appendix to 1e-16 at both orders. Applied, it took figure 8a's flexural **1.29 % → 0.063 %**, its screw **0.43 % → 0.058 %**, and figures 2a/7a's fast flexural **0.78/1.03/0.87 % → 0.23/0.38/0.44 %**. Reverted because it is half the change: the layered paths carry the same ansatz in separate code, so correcting the open hole alone breaks five `layer == formation` collapse-to-unlayered invariants and leaves the solver worse than either endpoint. Remaining: 63 SV-column references across `_cased.py` and `_n1_layered.py`, ~36 test updates, a golden regeneration, and figures 20-21 digitised to validate the cased result. Almost certainly also A.7's cause. Pinned by thirteen tests. |
